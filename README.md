...
```python
from nl.nl import ask, OllamaProvider
...
```python
from nl.nl import ask, OpenAIProvider
...
```python
from nl.nl import ask, OpenAICompatProvider
import os
...
```python
from nl.nl import ask, Provider
...
```python
from nl.nl import check_rules, OllamaProvider
...
| Fix | Detail |
|---|---|
| Default model upgraded | `llama3` → `deepseek-r1:latest` (8b) |
...
| Ollama auto-detection | `_default_provider()` now checks for a running Ollama instance first — no API key needed for local use |
